...
export const CameraInput: React.FC<CameraInputProps> = ({ onAnalyze, onCancel }) => {
  ...
  useEffect(() => {
    if (mode !== 'camera' && isCameraOpen) closeCamera();
  }, [mode, isCameraOpen]);
  ...
  return (
    <div className="flex flex-col h-full bg-white max-w-md mx-auto relative">
      ...
      <div className="flex-1 overflow-y-auto p-6 space-y-6">
        <div className="flex bg-gray-100 p-1 rounded-xl">
          <button
            ...
            className={`flex-1 py-2 text-sm font-bold rounded-lg transition-all flex items-center justify-center gap-2 ${
              mode === 'camera' ? 'bg-white shadow-sm text-sky-500' : 'text-gray-500'
            }`}
          >
            <Camera size={16} /> 写真
          </button>
          <button
            ...
            className={`flex-1 py-2 text-sm font-bold rounded-lg transition-all flex items-center justify-center gap-2 ${
              mode === 'text' ? 'bg-white shadow-sm text-sky-500' : 'text-gray-500'
            }`}
          >
            <Type size={16} /> テキスト
          </button>
        ...
        {mode === 'camera' && (
          <div className="space-y-4">
            ...
            {cameraError && (
              <div className="text-xs text-red-500 bg-red-50 border border-red-100 p-3 rounded-xl">{cameraError}</div>
            )}
      ...
      <input type="file" accept="image/*" ref={albumInputRef} onChange={handleFileChange} className="hidden" />
